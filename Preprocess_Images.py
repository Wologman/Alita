...
def make_final_labels(in_labels, debug=False, reduce_factor=None):
    ...
    out_labels = in_labels[['File_Path', 'Cropped_File_Name', 'Species', 'Camera']].copy()
...
def crop_save_image(row):
    ...
    try:
        ...
        cv2.imwrite(str(out_fn), img)  #, [int(cv2.IMWRITE_JPEG_QUALITY), 95] #Not sure what's going wrong with this
 
        #img_pil = Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))  
        #img_pil.save(out_fn, 'JPEG', quality=95)   #Works, but some images look corrupt.
