...
class Paths:
    LAST_MD_FOLDER_NM = 'MD_Last_Run'
    LABELS_FROM_JSON_NM = 'all_labels.parquet' #Output label file.
    EXIF_DATA_NM = 'last_exif_data.parquet' #Output exif data file
    DATA_FOLDER_NM = 'Data'  #Directory for all data folders
    EXPS_FOLDER_NM = 'Experiments'
    INPUT_FOLDER_NM = 'Inputs'
    def __init__(self, experiment_name, source_images):
        self.project_dir = Path(__file__).resolve().parent.parent
        self.data_folder = self.project_dir / self.DATA_FOLDER_NM 
        self.output_path = self.data_folder / self.EXPS_FOLDER_NM / experiment_name / self.INPUT_FOLDER_NM / self.LABELS_FROM_JSON_NM
        self.last_run_path = self.data_folder / self.EXPS_FOLDER_NM / self.LAST_MD_FOLDER_NM / self.LABELS_FROM_JSON_NM
        self.exif_path = self.data_folder / self.EXPS_FOLDER_NM / self.LAST_MD_FOLDER_NM  / self.EXIF_DATA_NM
        self.json_file_folder = self.data_folder / self.EXPS_FOLDER_NM / experiment_name / self.INPUT_FOLDER_NM
        self.json_paths = [f for f in self.json_file_folder.glob('*.json')]
        print(f'json files found in {self.json_file_folder}', self.json_paths)
        self.data_root = Path(source_images)
...
def get_class_location(filepath: str, classes: list[str]) -> tuple[str, str]:
    """
    Parses the file path to identify:
    - A class name that matches one in the given list (case-insensitive).
    - A unique 3-letter uppercase string representing the camera location.

    If no matching class is found, returns 'unknown'.
    If multiple or no potential location codes are found, location is 'unknown'.

    Args:
        filepath (str): The path to parse.
        classes (list of str): List of valid class names (lowercase preferred).

    Returns:
        tuple[str, str]: (class_name, location_code)
    """
    folder_names = re.split(r'[\\/]', filepath)
    lowered_names = [name.lower() for name in folder_names]

    # Identify location codes: exactly 3 uppercase letters
    potential_locations = [name for name in folder_names if re.fullmatch(r'[A-Z]{3}', name)]
    location = potential_locations[0] if len(potential_locations) == 1 else 'unknown'

    if classes:
        class_name = next((cls for cls in classes if cls.lower() in lowered_names), 'unknown')
    elif location != 'unknown':
        try:
            idx = folder_names.index(location)
            class_name = lowered_names[idx + 1] if idx + 1 < len(folder_names) else 'unknown'
        except ValueError:
            class_name = 'unknown'
    else:
        class_name = 'unknown'
    ...
    for detects in iterate_json(j_array, best_only, image_folder):
        if detects:
            for one_thing in detects:
                ...
                new_dict = {col:val for col, val in zip(columns,new_row)}
                data.append(new_dict)
    return data


def process_all_jsons(json_path_list, classes, image_folder=None, best_only=True):
    """Iterates through a list of json files and appends the result from each to a single dataframe which is returned"""
    
    data_list = []
    for json_path in json_path_list: 
        new_entry = process_json_array(json_path, classes, image_folder, best_only)
        data_list.extend(new_entry)
    ...
    if __name__ == '__main__': 
        unknown_class = df['Species'].value_counts().get('unknown', 0)
        unknown_place = df['Location'].value_counts().get('unknown', 0)
        species = df['Species'].nunique() - (unknown_class!=0)
        places = df['Location'].nunique() - (unknown_place!=0)
        print('The first three rows of the dataframe extracted from the MegaDetector predictions:')
        print(df.head())
        ...
        print(f'{len(df)} rows from the MegaDetector Runs')
    return df
...
def extract_exif_data(image_path: Union[str, Path]) -> ExifData:   
    #https://exiv2.org/tags.html    #306=DateTime, #36867=DateTimeOriginal 270=ImageDescription
    try:
...
def main(settings_pth=None):
    ...
    if not cfg.UPDATE_META_DATA:
        return
    paths=Paths(cfg.EXPERIMENT_NAME, cfg.SOURCE_IMAGES_PTH)

    if cfg.CLASSES_FROM_DIR_NMS:
        class_list = get_class_list(paths.data_root)
    else: class_list = cfg.CLASSES
    all_processed = process_all_jsons(paths.json_paths, class_list,  paths.data_root, cfg.BEST_ONLY)
    df_from_json = remove_missing_dirs(paths.data_root, all_processed)
    df_from_json = remove_old_filepaths(paths.data_root, all_processed)
    ...
    rat_folders = [grandchild for grandchild in paths.data_root.glob('*/*') if grandchild.is_dir() and grandchild.name == 'rat']
    rat_count = 0
    ...
    exif_df = get_last_exif_data(paths.exif_path)
    ...
    if cfg.UPDATE_EXIF:
        ...
        tasks = [delayed(extract_exif_data)(fp) for fp in file_paths]
        new_exif_data = Parallel(n_jobs=8, prefer='threads')(tasks)

        new_exif_df = pd.DataFrame(new_exif_data)  #the generator here is creating a typing error warning.
    ...
    exif_df.to_parquet(paths.exif_path) # Updates the 'MD_Last_Run' folder
    data_out.to_parquet(paths.output_path) # Updates the labels file before cleaning step.
    data_out.to_parquet(paths.last_run_path) # Updates the 'MD_Last_Run' folder

    ########################################################3
    ## Figure out a way to add a model name and date run column in the data parquet
    ########################################################3
