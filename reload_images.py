...
def run_mega_detector_old(process_folder, json_pth, run_md_pth):
...
def run_mega_detector(folder_path, json_out_path, model_name):
    detect(external_image_dir=folder_path, 
...
def get_unprocessed(root_fldr, exclude_fldrs, finished_pths):
    ...
    including_empty = len(new_paths)
    new_paths = [file for file in tqdm(new_paths, desc="Removing empties") if file.parent.name !='empty']
    new_paths = [file for file in tqdm(new_paths, desc="Removing empties") if file.parent.parent.name !='empty']
    print(f'{including_empty - len(new_paths)} empty files were removed')
...
def find_and_move_new_images(temp, last_md_folder, source_folder, independent_folders):
    ...
    2. 
    """
    #remove this while debugging, but in general, it would be safer to delete the temp folder automatically
    #if temp.is_dir():
    #    shutil.rmtree(temp)
    #    os.mkdir(temp)
    ...
    already_moved = set(temp.rglob("*.*"))
    unprocessed_paths = list(set(unprocessed_paths) - already_moved)
...
def main(settings_pth = None):
    ...
    weights_path = project_folder / 'models/md_v5a.0.0.pt'
    ...
    if cfg.RERUN_MD_ON_ALL:
        print(f'Rerunning Megadetector on {source_folder}')
        run_megadetector(source_folder,
                    detector_weights_pth=weights_path,
                    json_path=json_pth,
                    detector_name='md_v5',
...
if __name__ == '__main__':
    main('C:/Users/User/OneDrive - Department of Conservation/Desktop/Alita/settings/Exp_49_Run_01.yaml')