...
class Paths:
    ...
    def __init__(self,
        ...
        check_weights('MegaDetector', [path for path in self.project_dir.rglob(self.MD_WEIGHTS_NM) if path.is_file()])
...
def run_megadetector(image_dir: Path, 
    ...
    if not json_pths:
        detect(external_image_dir=image_dir,
               model_name=detector_name,
               detector_weights_pth= detector_weights_pth,  # detector_weights_pth,
...
def make_results_table(results_dict, 
                       ...
                       empty_classify_threshold=0.5, #If nothing over this, we predict 'unknown if md_empty is met'
                       hard_classes=[],
                       empty_string = 'Empty',
