...
class DefaultConfig:
    ...
    MODEL_WEIGHTS_PATH = r'C:\Users\User\OneDrive - Department of Conservation\Desktop\Alita\models\Exp_46\Exp_46_Run_21_best_weights.pt'
...
def encode_images(images_df, model, image_cfg):
    ...
    transforms = ImagePrep(mean = image_cfg.INPUT_MEAN,
...
class CustomModel(pl.LightningModule):
    def __init__(self,
        ...
        # Replace the classifier head with Identity for embedding output
        self.backbone.classifier = nn.Identity()
        ...
        if weights:
            ...
            if 'state_dict' in state_dict:  # if it's a Lightning checkpoint
                ...
                # remove 'backbone.' or 'model.' prefix if needed
                new_state_dict = {}
                for k, v in state_dict.items():
                    if k.startswith('backbone.'):
                        new_state_dict[k[len('backbone.'):]] = v
                    elif not k.startswith('loss') and not k.startswith('classifier'):
                        new_state_dict[k] = v
...
def get_encoder(weights=None):
    network = CustomModel(weights=weights)
    device, gpu = test_cuda()
    network.to(device)
    return network


def get_image_embeddings(current_df, h5_path, image_config, model_weights):
    if h5_path.exists():
    ...
    if missing_embeddings:
        model = get_encoder(weights=model_weights)
...
def limit_with_embeddings(df, 
    ...
    embedding_dict = get_image_embeddings(df, h5_path, image_config=img_cfg, model_weights=model_weights)
...
def main():
    cfg = DefaultConfig()
    img_cfg = ImageConfig()
    root = Path(r'C:\Users\User\OneDrive - Department of Conservation\Desktop\Alita\data\experiments\MD_Last_Run')
    df_path = root / 'all_labels.parquet'
    hash_h5_path = root / 'image_hashes.h5'
    hash_dist_h5_path = root / 'hash_pair_distances.h5'
    embedding_h5_path = root / 'image_embeddings.h5'
    ...
    df = limit_with_embeddings(df,
                               ...
                               img_cfg=img_cfg,
                               limit=cfg.MAX_PER_CLASS_PER_LOCATION,
                               model_weights = cfg.MODEL_WEIGHTS_PATH)
