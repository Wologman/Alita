"""
run_detector_batch_modified.py

A slightly adjusted version to take arguments from Detection.py
To call this from another script
from megadetector.detection.run_detector_batch_modified import run_megadetector

# Create a strin list of args and flags as if they were going to be used in a shell terminal
args = [r'E:/Alita/Models/md_v5a.0.0.pt',
...
The argsparser will sort the args into variables as in normal MD use
megadetector  (from https://pypi.org/project/megadetector/)
yolov5        (from version ultralytics-yolov5-0.1.1)

Put these folders in the Python directory along side the other scripts 
...
#def main():
def run_megadetector(cli_args=None):
    ...
    import warnings
    warnings.filterwarnings("ignore", message=".*NMS time limit.*")  #needed to stop annoying warning on slower machines
